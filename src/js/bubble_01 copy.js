...
function applyFilter() {
  ...
  const currentCityLimit = parseInt(document.getElementById('applyFilterBtn').value, 10) || 10;
  ...
  stackedBarChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: `Top ${labels.length} Cities by Function`,
          font: {
            size: 14
          },
          color: '#1C1DAB',
...
document.getElementById('toggleChartsBtn').addEventListener('click', () => {
  ...
  const wrapper = document.getElementById('dashboardWrapper');  // 👈 确保 ID 正确

  const isExpanded = bg.classList.toggle('expanded');
  charts.classList.toggle('expanded', isExpanded);

  if (wrapper && isExpanded) {
    wrapper.classList.add('scrollable');  // ✅ 展开时允许滚动
  } else {
    wrapper.classList.remove('scrollable');  // ✅ 收起时不滚动
  }
